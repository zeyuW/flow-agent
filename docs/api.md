...
### 2.3 进程内、用户工作区和插件附带的服务

MCP 服务来源可以是：

- 内置服务：由应用启动时提供，适合系统必需的能力。
- 用户工作区服务：写入 `~/.flow/mcp.json`，适合本机部署环境或个人使用的外部工具。
...
Flow Agent 默认注册一组本地工具，用于在受控边界内读写项目和用户运行目录：

| 工具 | 用途 | 默认风险 |
| --- | --- | --- |
| `read` | 读取 UTF-8 文本文件 | 只读 |
| `write` | 创建或覆盖文本文件 | 写入 |
| `edit` | 精确替换文件中的一段文本 | 写入 |
| `bash` | 在项目根目录执行命令 | 写入/外部副作用 |
| `undo` | 撤销受支持的最近一次变更 | 写入 |
| `install_skill` | 从 Git 仓库安装用户 Skill | 写入 |
| `spawn` / `task` | 创建后台任务或隔离 Subagent | 受策略限制 |
| `mcp` 管理工具 | 查询和管理外部 MCP 连接 | 外部副作用 |

工具必须先注册，再经过 `ToolGuard` 检查工具名称和输入，模型不能通过未注册名称
绕过工具策略。工具调用的风险等级、尝试次数和结果都会进入回合事件或追踪边界；
高风险工具默认不自动重试。
...
相对路径       --> 仓库根目录
.flow/<path>   --> ~/.flow/<path>
绝对路径       --> 按绝对路径访问，并继续接受工具和系统权限限制
```

因此，项目代码、配置模板和共享 Skill 应使用普通项目相对路径；会话数据库、记忆、
日志、追踪、插件、用户 Skill、MCP 配置和 Subagent 任务应使用 `.flow/` 逻辑路径。
路径映射不是安全授权本身，读取敏感文件、执行命令和访问外部系统仍须经过工具守卫、
操作系统权限和相应服务端校验。

用户 Skill 的安装目标固定为 `~/.flow/skills/`，不会写入其他 Agent 的技能目录；
项目共享 Skill 使用仓库根目录的 `skills/`。

### 4.2 工具扩展的验证

新增本地工具或改变工具输入时，至少验证：工具能被注册和发现、缺少必填参数会
返回可理解的失败、危险路径会被拒绝、执行异常不会泄漏秘密，并且风险等级与重试
策略符合副作用。需要独立进程或外部系统的能力应优先实现为 MCP，而不是扩大
`bash` 的隐式权限。
