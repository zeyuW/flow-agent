...
AfterTurn 阶段顺序（确保正确性）：
  ...
  ② MessageSender.send(SendMessage) - 后通过发送端口投递回复
...
class PassiveTurnPipeline:
    ...
    def __init__(
        ...
        # 迁移期间保留旧出站端口兜底；新组合根统一注入 MessageSender。
    ...
    def process(self, inbound: IncomingMessage) -> None:
        ...
        管道处理完成后：
        ① 通过 EventBus 广播 TurnCommitted 事件
        ② 通过 MessageSender 投递回复到 MessageBus 出站队列
    ...
    def _after_turn(self, flow: TurnFlow) -> TurnFlow:
        ...
        - 再通过 MessageSender 发送回复，确保回复进入可靠队列
    ...
    def _send_outbound_reply(self, flow: TurnFlow) -> None:
        """通过 MessageSender 投递出站回复。

        将管道的 final_output 封装为 SendMessage，
        通过 MessageSender.send() 投递到 MessageBus 出站队列。
