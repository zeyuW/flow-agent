    ...
    async def scenario():
        ...
        class Pipeline:
            async def process_async(self, inbound):
                if inbound.conversation_id == "session-1":
                    first_started.set()
...
def test_passive_pipeline_allows_other_session_while_async_model_waits():
    ...
    async def scenario():
        ...
        first = asyncio.create_task(
            ...
            IncomingMessage(channel="cli", conversation_id="session-1", text="first")
            )
        )
        ...
        second = asyncio.create_task(
            pipeline.process_async(
                IncomingMessage(channel="cli", conversation_id="session-2", text="second")
            )
        )
...
def test_async_pipeline_applies_tool_hook_before_execution():
    ...
    async def scenario():
        ...
        await pipeline.process_async(
            IncomingMessage(channel="cli", conversation_id="session-1", text="测试")
        )
