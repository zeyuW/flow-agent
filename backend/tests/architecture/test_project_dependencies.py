...
def test_shared_infrastructure_has_no_business_dependency():
    violations = [
        ...
        for target in targets
        if target.startswith(("modules.", "interfaces.", "bootstrap.", "flow_agent."))
    ]
    assert sorted(violations) == []


def test_migration_package_does_not_depend_on_adapters_or_composition_root():
    violations = [
        (source, target)
        for source, targets in graph().items()
        if source.startswith("flow_agent.")
        for target in targets
        if target.startswith(("interfaces.", "bootstrap."))
    ]
    assert sorted(violations) == []
